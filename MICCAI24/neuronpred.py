...
from src.data.hemibrain import HemiBrain
from src.data.manc import Manc
from src.data import data_loader
from src.models.GraphEncoder.flygcl import GConv,Encoder
...
def main():
    ...
    if(dataset_name.upper() == "HEMIBRAIN"):
        dataset = HemiBrain(path, transform=transform)
    if(dataset_name.upper() == "MANC"):
        dataset = Manc(path, transform=transform)
    ...
    gconv = GConv(input_dim=dataset.num_features, hidden_dim=hidden_dim, activation=torch.nn.ReLU, num_layers=num_layers, node_num=dataset.x.size(0)).to(device)
    ...
    for epoch in range(1, num_epochs):
        loss = train.train_encoder(encoder_model, contrast_model, train_data.x,train_data.edge_index, optimizer)
        if epoch % 10==0:
            with torch.no_grad():
                encoder_model.eval()
    ...
    train_loader = data_loader.NeuronClassDataLoader({'batch_size':1000,'device':device},train_data,z)

  
    config = {'input_channels':hidden_dim,'dropout':0.3,'out_channels':train_data.y.max()+1}
    ...
    for ep in tqdm.tqdm(range(500)):
        train.train_classify(pair_deco,train_loader,opt,z,classifyloss)
    test_result = evaluate_NeuClassify(pair_deco, test_data,z,device=device)
    print(test_result)
